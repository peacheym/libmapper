...
//     parent = mpr_dev_new("test-parent", 0);
//     if (!parent)
//         goto error;
//     eprintf("parent created.\n");

//     src = mpr_obj_add_child_dev((mpr_obj)parent, "test-send", 0);
//     if (!src)
//         goto error;
//     eprintf("source (child) created.\n");
...
// {
//     while (!done && !(mpr_dev_get_is_ready(src) && mpr_dev_get_is_ready(dst) && mpr_dev_get_is_ready(parent)))
//     {
...
int main(int argc, char **argv)
{
    ...
    parent_dev = mpr_dev_new("test-parent", 0);
    parent_dest = mpr_dev_new("test-dest", 0);

    mpr_dev child_dev1 = mpr_obj_add_child_dev((mpr_obj)parent_dev, "test-child1", 0);
    mpr_dev child_dev2 = mpr_obj_add_child_dev((mpr_obj)parent_dev, "test-child2", 0);
    mpr_dev grand_child_dev = mpr_obj_add_child_dev((mpr_obj)child_dev1, "test-grand-child", 0);

    float mnf[] = {3.2, 2, 0}, mxf[] = {-2, 13, 100};
    double mnd = 0, mxd = 10;

    mpr_sig sendsig_1 = mpr_sig_new(parent_dev, MPR_DIR_OUT, "outsig_1", 1, MPR_DBL, "Hz",
                                    &mnd, &mxd, NULL, NULL, 0);

    float mnf2[] = {0, 0, 0}, mxf2[] = {1, 1, 1};

    mpr_sig recvsig_1 = mpr_sig_new(grand_child_dev, MPR_DIR_IN, "insig_1", 1, MPR_FLT, NULL,
                                    mnf2, mxf2, NULL, handler, MPR_SIG_UPDATE);

    while (!(mpr_dev_get_is_ready(parent_dev) && mpr_dev_get_is_ready(child_dev1) && mpr_dev_get_is_ready(child_dev2) && mpr_dev_get_is_ready(grand_child_dev)))
    {
        mpr_dev_poll(parent_dev, 0);
        mpr_dev_poll(child_dev1, 0);
        mpr_dev_poll(child_dev2, 0);
        mpr_dev_poll(grand_child_dev, 0);
        mpr_dev_poll(parent_dest, 0);
        // mpr_dev_poll_all_children(parent_dev);
    }

    printf("All Devices Ready!\n");

    mpr_map map = mpr_map_new(1, &sendsig_1, 1, &recvsig_1);

    mpr_obj_push(map);
    printf("Map signals!", mpr_map_get_is_ready(map));
